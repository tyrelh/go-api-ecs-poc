...
parser = argparse.ArgumentParser(description="Deploy Go API POC ECR (Elastic Container Repository) stack")
parser.add_argument(
    "--aws-account",
    type=str,
...
stack = "go-api-poc-ecr"
template_file = "ecr-stack.yml"
if args.ci:
...
command = f'aws cloudformation deploy --template-file {template_file} --stack-name {stack} --parameter-overrides "AwsAccount={args.aws_account}" --profile {profile} --region {args.aws_region}'
